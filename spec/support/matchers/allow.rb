RSpec::Matchers.define :allow do |*values|
  ...
  failure_message_for_should do |record|
    "expected #{attribute_description} to be valid " <<
    "with #{values.inspect} (failed: #{@failed_values.inspect})"
  end

  failure_message_for_should_not do |record|
    "expected #{attribute_description} to be invalid " <<
    "with #{values.inspect} (passed: #{@failed_values.inspect})"
  end
